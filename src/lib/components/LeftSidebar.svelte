<script lang="ts">
    ...
    const DEFAULT_WORDPRESS_LOGIN_URL = `${WORDPRESS_ORIGIN}/wp-login.php?redirect_to=${encodeURIComponent(
        WORDPRESS_ADMIN_URL,
    )}`;
    ...
    const wordpressPageLinks: ExternalNavItem[] = [
        {
            ...
            href: `${WORDPRESS_ORIGIN}/wp-admin/edit.php?post_type=ankuendigung`,
        },
        {
        ...
        {
            label: "Veranstalter",
            href: `${WORDPRESS_ORIGIN}/wp-admin/edit.php?post_type=tribe_organizer`,
        },
        {
            label: "Tanzraeume",
            href: `${WORDPRESS_ORIGIN}/wp-admin/edit.php?post_type=tribe_venue`,
        },
    ];
    ...
    const wordpressStatusLabel = $derived.by(() => {
        ...
        if (!wordpressStatus.available)
            return "WordPress-Status nicht verfuegbar";
        return wordpressStatus.loggedIn
            ? "Bei WordPress eingeloggt"
...
>
    ...
    <div class="shrink-0 px-4 pt-6 pb-4">
        ...
        >
            <div
                class="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden border border-border-default bg-surface-card shadow-card"
            >
                <img
                    src="/rnt-logo.png"
                    alt=""
            ...
            <div class="min-w-0">
                <p
                    class="font-display text-[1.25rem] font-semibold text-text-default truncate"
                >
                    RNT Kalender
                </p>
                <p class="text-[0.8125rem] text-text-muted truncate">
                    Rhein-Neckar-Tango
                </p>
            </div>
        </a>
