...
{#if pwaInstallStore.modalOpen}
	...
	>
		...
		>
			<Card variant="elevated" padding="lg" radius="lg" shadow="lg">
				<div class="flex items-start justify-between gap-4">
					<div class="space-y-1">
						...
						<Text size="lg" color="muted">{content.intro}</Text>
					</div>
					<Button
						onclick={close}
						variant="ghost"
				...
				{#each content.steps as step, index (index)}
					...
					>
						...
						<div class="flex-1 space-y-1">
							<Text size="base" color="default" as="p">{step.text}</Text>
			...
			{#if content.note}
				<Text size="lg" color="muted" as="p" class="mt-4">{content.note}</Text>
			{/if}
