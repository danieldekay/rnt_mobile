...
export async function subscribe(
	...
	return toSignupResponse(await subscribeNewsletter(email, hp, nonce));
}

export async function unsubscribe(email: string): Promise<SignupResponse> {
	const nonce = await getNewsletterNonce();
	return toSignupResponse(await unsubscribeNewsletter(email, nonce));
}
...
export async function verifyEmail(token: string): Promise<SignupResponse> {
	...
	return {
		...
		error: "unsupported",
		message:
			"Die Bestaetigung wird direkt ueber den Link in der Sendy-E-Mail verarbeitet.",
	};
}
