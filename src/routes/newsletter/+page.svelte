<script lang="ts">
    ...
    onMount(async () => {
        ...
        if (verifyToken) {
            verifyOk = ["verified", "success"].includes(
                verifyToken.toLowerCase(),
            );
    ...
    async function handleSubscribe(event: SubmitEvent) {
        ...
        try {
            ...
            message =
                err instanceof Error
                    ? err.message
                    : "Die Anmeldung war gerade nicht moeglich.";
            trackFeatureEvent("newsletter", "error");
        }
    }
    ...
    async function handleUnsubscribe() {
        ...
        try {
            ...
            message =
                err instanceof Error
                    ? err.message
                    : "Die Abmeldung war gerade nicht moeglich.";
        }
    }
    ...
    <section class="card p-5" aria-labelledby="newsletter-heading">
        <div class="space-y-1 mb-5">
            ...
            <p class="meta-text max-w-[48ch]">
                Kurze Updates zu neuen Funktionen, wichtigen RNT-Hinweisen und
                Terminen. Abmeldung jederzeit ueber den Link in jeder Mail.
            </p>
        </div>
        ...
        <div class="space-y-4">
            ...
            {:else if pageState === "unconfirmed"}
                ...
                >
                    <p class="text-[0.9375rem] font-medium text-text-default">
                        {message ||
                            "Bestaetigung ausstehend. Bitte pruefe dein Postfach und klicke den Bestaetigungs-Link."}
                    </p>
                    <button
                        class="btn-secondary"
                        type="button"
            ...
            {:else if canSubscribe}
                <form onsubmit={handleSubscribe} novalidate class="space-y-3">
                    <label class="flex cursor-pointer items-start gap-3">
                        ...
                        />
                        <span
                            class="text-[0.875rem] leading-relaxed text-text-muted"
                        >
                            Ich habe die
                            <a
